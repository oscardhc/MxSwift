    ...
    override func visit(node: VariableD) {
        ...
        if curBlock == nil {
            node.variable.forEach {
                curClass!.added(subType: ($0.0, type))
            }
        } else {
            node.variable.forEach {
...
//        super.visit(node: node)
    ...
    override func visit(node: PropertyAccessE) {
        super.visit(node: node)
        print(">>>>", node.toAccess.type)
        let cls = node.scope.find(name: node.toAccess.type) {
            $0.subScope?.scopeType == .CLASS
        }
        var count = 0
        for pro in (cls!.subScope!.correspondingNode! as! ClassD).properties {
            for (j, (name, _)) in pro.variable.enumerated() {
                if name == node.property {
                    node.ret = GEPInst(name: "",
                                       type: getType(type: pro.type),
                                       base: node.toAccess.ret!,
                                       needZero: false,
    ...
    override func visit(node: NewE) {
        ...
        let call = CallInst(name: "", function: Builtin.malloc(), arguments: [IntInstant(name: "", type: IRInt.long, value: size)], in: curBlock!)
